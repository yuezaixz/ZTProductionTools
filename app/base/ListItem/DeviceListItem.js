        ...
        return (
            <TouchableHighlight
                ...
                <View style={[styles.container, {borderBottomWidth: this.props.isLast?0:1}]}>
                    <Text style={styles.name}>{this.props.onlyStrong?'Deeper Pillow':((this.props.data.localName || this.props.data.name) + '📶  ' + (this.props.data.rssi+130) )}</Text>
                    <Text style={[styles.body, styles.timer]}>Click to connect</Text>
                </View>
            </TouchableHighlight>
        );
    }
